...
- `SequentialBatchProcessor`：基于 consumer 的 processor，在调用线程中按输入顺序
  处理数据项。
- `ParallelBatchProcessor`：基于 consumer 的 processor，使用 scoped 标准线程处理数据项。
- `ChunkedBatchProcessor`：按固定大小拆分逻辑批次，并把每个 chunk 提交给代理
  processor 的适配器。
- `SequentialBatchExecutor`：在调用线程中按顺序执行任务。
- `ParallelBatchExecutor`：基于 scoped 标准线程的固定宽度并行执行器。
- `ProgressReporter`：`qubit-progress` 的进度上报接口，接收结构化
  `ProgressEvent` 生命周期通知。
- `WriterProgressReporter` 和 `LoggerProgressReporter`：从 `qubit-progress`
  重导出的 writer 和 `log` crate 进度上报器。
...
- 能检测声明任务数与迭代器实际产出数量不一致的情况，并携带已经收集到的
  部分执行结果。
- 核心 API 不依赖具体运行时，同时提供一个标准库并行执行器。
...
当每个数据项都能由 `qubit-function` 的 `Consumer` 处理时，可以直接使用
`SequentialBatchProcessor` 或 `ParallelBatchProcessor`。
...
- `SequentialBatchExecutor::new()` 是确定性的，会在调用线程中按迭代器顺序执行
  任务。
- `ParallelBatchExecutor::default()` 使用可用 CPU 并行度和 scoped 标准线程，
  因此任务可以借用调用者栈上的数据。
- `BatchOutcome::failures()` 返回按任务下标排序的失败记录。
- `BatchTaskFailure::index()` 是从 0 开始的下标，对应任务在批次中的位置。
- 核心 crate 刻意避免运行时依赖；可用 `ParallelBatchExecutor` 做标准库固定宽度
  并行执行，如果需要基于 Rayon 的 CPU 执行，可使用配套的 `qubit-rayon-batch`
  crate。
...
- `ParallelBatchExecutor`：基于 scoped 标准线程的固定宽度并行执行器，不引入运行时依赖。
- `BatchProcessor`：处理一批声明数量的数据项的 trait。
- `BatchProcessResult`：聚合处理结果，包含数据项数量、已处理数量、chunk 数量和单调耗时。
- `BatchProcessError`：内置 consumer-backed processor 返回的输入数量不匹配错误。
- `SequentialBatchProcessor`：基于 `BoxConsumer` 的 processor，按输入顺序调用
  consumer。
- `ParallelBatchProcessor`：基于 `ArcConsumer` 的 processor，使用固定宽度 scoped
  标准线程调用 consumer。
...
- `ProgressReporter`：`qubit-progress` 的 trait，接收 `ProgressEvent`，用于批次
  开始、周期性进度和终态通知。
- `NoOpProgressReporter`：默认进度上报器，接收回调但不执行任何操作。
- `WriterProgressReporter` 和 `LoggerProgressReporter`：
  从 `qubit-progress` 重导出的 writer 和 `log` crate 进度上报器。
...
- `src/processor`：数据项批处理 trait、结果类型、consumer-backed processor 和
  chunked processor。
- `src/progress`：从 `qubit-progress` 重导出的进度事件和上报器类型。
- `tests/executor`：顺序执行、进度回调、失败、panic 和数量不匹配的行为测试。
- `tests/processor`：direct processor、chunked processor、错误和进度行为测试。
