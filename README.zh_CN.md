...
- `ConsoleProgressReporter` 和 `LoggerProgressReporter`：分别输出到 stdout 和
  `log` crate 的具体进度上报器。
- `BatchExecutionResult`：结构化的批处理结果，包含失败聚合和单调耗时信息。
...
- `WriterProgressReporter`、`ConsoleProgressReporter` 和 `LoggerProgressReporter`：
  分别向 writer、stdout 和 `log` crate 输出进度信息的具体上报器。
- `BatchExecutionResult<E>`：聚合执行结果，包含任务计数、单调耗时和详细失败记录。
...
- `src/progress`：进度上报 trait、no-op、stdout、writer 和 logger 上报器。
